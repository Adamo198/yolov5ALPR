...
from utils.alpr_utils import check_file_name, crop_prepare
...
def save_one_box(xyxy, im, file=Path("im.jpg"), gain=1.02, pad=0, square=False, BGR=False, save=True):
    ...
    crop_prepared = crop_prepare(crop)

    if save:
        file.parent.mkdir(parents=True, exist_ok=True)  # make directory
        f = str(check_file_name(file).with_suffix('.jpg'))
        # cv2.imwrite(f, crop)  # save BGR, https://github.com/ultralytics/yolov5/issues/7007 chroma subsampling issue
        Image.fromarray(crop_prepared[..., ::-1]).save(f, quality=95, subsampling=0)  # save RGB
    return f
