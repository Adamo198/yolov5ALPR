...
from utils.alpr_utils import anpr, alpr_init
...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        LOGGER.info(f"{s}{'' if len(det) else '(ALPR - no detections), '}{dt[1].dt * 1E3:.1f} ms, {(anpr_res if len(anpr_res) else '(ANPR - no valid detections)') if len(det) else '(ANPR - standby)'} {ocr_time * 1E3:.1f} ms\n")
    ...
    LOGGER.info(f"Correct license plate detections: {det_count}")
